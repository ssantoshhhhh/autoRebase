...
export default function ComplaintDetailPage() {
  ...
  const addNote = async () => {
    ...
    try {
      await api.post(`/api/police/complaints/${id}/notes`, { note }, { headers });
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--clr-bg)' }}>
      ...
      >
        <button className="btn btn-ghost btn-sm" onClick={() => navigate('/police/dashboard')}>
          ← Back
        </button>
        <div
          style={{
      ...
      >
        ...
        <div>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  color: activeView === tab.id ? 'var(--clr-text)' : 'var(--clr-text-muted)',
                  borderBottom:
                    activeView === tab.id
          ...
          {activeView === 'transcript' && (
            <div className="card">
              <h4 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>Full Transcript</h4>
              <div
                style={{
          ...
          {activeView === 'extraction' && (
            <div className="card">
              <h4 style={{ marginBottom: '16px', fontSize: '0.95rem' }}>Extracted Information</h4>
          ...
          {activeView === 'timeline' && (
            <div className="card">
              <h4 style={{ marginBottom: '16px', fontSize: '0.95rem' }}>Activity Timeline</h4>
                  ...
                  <div key={i} style={{ marginBottom: '20px', position: 'relative' }}>
                    <div
                      style={{
                        ...
                        height: '10px',
                        borderRadius: '50%',
                        background: u.type === 'EMERGENCY_FLAG' ? '#ff3b30' : 'var(--clr-primary)',
                        top: '4px',
          ...
          <div className="card" style={{ marginTop: '16px' }}>
            <h4 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>Add Internal Note</h4>
            <textarea
              id="note-input"
              className="input"
              value={note}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {/* Complaint Info */}
          <div className="card">
            <h4 style={{ marginBottom: '16px', fontSize: '0.95rem' }}>Details</h4>
            <div style={{ display: 'grid', gap: '10px', fontSize: '0.85rem' }}>
              <InfoRow label="Incident Type" value={complaint.incidentType || 'General'} />
              <InfoRow
                label="Location"
                value={
              ...
              <InfoRow label="Anonymous" value={complaint.isAnonymous ? 'Yes' : 'No'} />
            </div>
          </div>
          ...
          {!complaint.isAnonymous && complaint.user && (
            <div className="card">
              <h4 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>Complainant</h4>
              <div style={{ display: 'grid', gap: '8px', fontSize: '0.85rem' }}>
                ...
                <InfoRow label="Aadhaar" value={complaint.user.aadhaarMasked || 'N/A'} />
                <InfoRow label="Total Complaints" value={complaint.user.complaintCount} />
                <InfoRow label="Risk Flags" value={complaint.user.riskFlagCount} />
              </div>
            </div>
          )}
          ...
          {/* Status Update */}
          <div className="card">
            <h4 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>Update Status</h4>
            <select
              id="status-select"
              className="input"
          ...
          {/* Officer Assignment */}
          {['STATION_ADMIN', 'SUPER_ADMIN'].includes(policeUser?.role) && officers.length > 0 && (
            <div className="card">
              <h4 style={{ marginBottom: '12px', fontSize: '0.95rem' }}>Assign Officer</h4>
              <p
                style={{
                  fontSize: '0.8rem',
                  color: 'var(--clr-text-muted)',
          ...
          {complaint.evidence?.length > 0 && (
            <div className="card">
              ...
              {complaint.evidence.map((e, i) => (
                ...
                >
                  <span style={{ color: 'var(--clr-text-muted)' }}>{e.fileType}</span>
...
function InfoRow({ label, value }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
      <span style={{ color: 'var(--clr-text-faint)', flexShrink: 0 }}>{label}</span>
      <span
        style={{
          color: 'var(--clr-text)',
          textAlign: 'right',
